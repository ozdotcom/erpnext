...
class TestTaxWithholdingCategory(unittest.TestCase):
	...
	def test_cumulative_threshold_tcs(self):
		...
		tcs_charged = sum(
			d.base_tax_amount for d in si.taxes if d.account_head == "TCS - _TC"
		)
	...
	def test_tcs_on_unallocated_advance_payments(self):
		...
		# assert tax collection on total invoice amount created until now
		tcs_charged = sum(
			d.base_tax_amount for d in si2.taxes if d.account_head == "TCS - _TC"
		)
		tcs_charged += sum(
			d.base_tax_amount for d in si3.taxes if d.account_head == "TCS - _TC"
		)
